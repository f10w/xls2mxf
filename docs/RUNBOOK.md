...
Пошаговый сценарий для оператора. Предполагается, что `copy_rollers.exe` и
`copy_rollers.conf` уже настроены (см. CONFIG.md).
...
Проверьте пути в `copy_rollers.conf` (секция `[paths]`). Можно разово
